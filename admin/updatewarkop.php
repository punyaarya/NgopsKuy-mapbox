                        ...
                        <div class="main-card mb-3 card">
                            ...
                            <div class="card-body">
                                ...
                                <form action="aksi_update.php" method="POST" enctype="multipart/form-data">
                                    ...
                                    include 'koneksi.php';
                                    $id = $_GET['id'];
                                    $sql = "SELECT * FROM data_location where id = $id";
                                    $hasil = $koneksi->query($sql);
                                    $nomor = 1;
                                    while ($warkop = $hasil->fetch_assoc()) { ?>
                                        <input name="id" type="hidden" class="form-control" value="<?= $warkop['id'] ?>">

                                        <label for="nama" class="">Nama warkop</label>
                                        <input name="nama" type="text" class="form-control" value="<?= $warkop['nama'] ?>">

                                        <label for="max_cap" class="">Kapasitas Maksimal</label>
                                        <input name="max_cap" type="text" class="form-control" value="<?= $warkop['max_cap'] ?>">

                                        <div class="divider"></div>

                                        <label for="lat" class="">Latitude</label>
                                        <input name="lat" type="text" class="form-control" value="<?= $warkop['lat'] ?>">

                                        <label for="lon" class="">Longitude</label>
                                        <input name="lon" type="text" class="form-control" value="<?= $warkop['lon'] ?>">
